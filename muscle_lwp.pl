...
IMPORTANT NOTE: This program was modified by Jair Santiago Garcia Sotelo and Kim Palacios FLores on 02/11/2017 to re-execute MUSCLE using a new JOB ID when it is not responding. 
...
$Id: muscle_lwp.pl 2791 2014-05-29 15:31:39Z hpm $
...
my $baseUrl = 'http://www.ebi.ac.uk/Tools/services/rest/muscle';
...
GetOptions(
    ...
    'tree|t=s'      => \$tool_params{'tree'},      # Tree
    'order|r=s'     => \$tool_params{'order'},     # Order of sequences in alignment
    'sequence=s'    => \$params{'sequence'},       # Input sequences/alignment

	# Compatiblity options (old command-line)
    'output|o=s'     => \$params{'output'},        # Output format
    'outputtree=s'   => \$params{'outputtree'},    # Output tree
...
sub load_params {
	...
	# Compatiblity options (old command-line)
	if(!$tool_params{'format'} && $params{'output'}) {
		$tool_params{'format'} = $params{'output'};
	}
...
# Check status and wait if not finished. Terminate if three attempts get "ERROR".
	my $errorCount = 0;
	#Modify by Jair Santigo Garcia Sotelo/Kim Palacios Flores 02/11/2017
	my $contRUNNING = 0;
...
sub get_results {
	...
	my $jobidINICIAL = shift;
	print_debug_message( 'get_results', 'jobid: ' . $jobidINICIAL, 1 );

	# Verbose
	if ( $outputLevel > 1 ) {
		print 'Getting results for job ', $jobidINICIAL, "\n";
	}
	...
	#Modified by Jair Santigo Garcia Sotelo/Kim Palacios Flores 02/11/2017
	 my ($jobid, $status) = client_poll($jobidINICIAL);
