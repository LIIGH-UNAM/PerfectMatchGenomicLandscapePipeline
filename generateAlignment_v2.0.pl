...
print "\nFormatting alignment files\n";
    ...
    while(readdir $muscleDirIndex){  
        ...
        if (-f ${muscleDir} . "/" .$file) { 
            ...
            if ($file =~ /.*\.aln-clustalw\.clw/) { 

                push (@muscleFileArray, $file); 

                $file =~ /(RGSL\d+_\d+_\d+)_F.*/;
                my $key = $1;

                $file =~ s/.aln-clustalw.clw//;

                if ($muscleFileUnique{$key}){
                    push(@{$muscleFileUnique{$key}}, $file);
                }else{
                    my $array = [];
                    push (@$array, $file);
                    $muscleFileUnique{$key} = $array;
                }
            }
        } 
    }
...
### No variation alignment files
print "\nSearching No variation alignment files\n";

    foreach my $key (keys %muscleFileUnique) {

        my $findNoVariationMotive = "true";

        foreach (@{$muscleFileUnique{$key}}) {

           my $file = $_;
            my @structureIdFile;

            print "\n\nLoading:  $file\n";  
            open(IN, ${alignmentDir}.$file) or die("Can't read ${alignmentDir}$file file");
                @structureIdFile = <IN>;
            close(IN);

            my $comparisonLine = $structureIdFile[5];

            if ($comparisonLine !~ /^\*+$/){
                $findNoVariationMotive = "false";
            }

        }

        if ($findNoVariationMotive eq "true"){
            open(OUTPUT_REPORT, ">> ${alignmentDir}noVariation.txt") or die("Can't create ${alignmentDir}noVariation.txt");
                foreach (@{$muscleFileUnique{$key}}) {
                    my $file = $_;
                    $fileComparationName = $file;
                    $fileComparationName =~ s/.fasta//;
                    $fileComparationName = $alignmentDir.$fileComparationName."_noVariation.fasta";
                    system ("mv ${alignmentDir}$file $fileComparationName");

                    print OUTPUT_REPORT $key."\n";
                }
            close(OUTPUT_REPORT);
        }
    }
